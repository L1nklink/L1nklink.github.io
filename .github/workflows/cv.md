...
#     - name: OPPO AI 挑战赛人像分割任务 决赛优秀奖
  ...
  experience:
    ...
    - name: 后端开发工程师 @ 上海瞬已网络科技有限公司
      time: 2021.05 - 至今
      content:
      - 核心网节点的控制进程开发, 负责与平台以及数据转发进程的消息协议设计、实现多生产者多消费者的消息无锁队列实现、基于共享内存的进程主备和数据缓存、平台下发的原始路径树解析、基于路径树和链路状态的自主实时路径选择算法、数据埋点等模块。
      - 核心网节点的数据转发模块的多线程转发功能的基础实现和后期性能调优。
      - 核心网的帧传输协议的部分开发工作,其中主要包括FEC网络编解码模块设计与实现、反馈链路发包参数的调整策略、内存池形式的进程内存管理等。
      - 完整参与核心网从开发到上线生产环境的全过程，并完成相关代码的维护和升级工作。对负责的代码，均完成开发、单元测试编写、设计测试用例、跟踪维护、按需求调优和文档记录工作。
      - 当前核心网已承载亚洲杯备用线路,MLB比赛全球范围转播和CCTV国际频道等多个相关业务,长时间保持低延迟平稳运行, CC ERROR发生率处于极低水平。
  ...
  coding:
    name: 开发经验
    content:
     - desc: 语言：熟练使用C、C#、C++11特性及部分后续新特性，可以使用脚本语言如python、lua完成部分工作的自动化。工作经验以C++为主。
     - desc: 数据结构：熟悉常用算法和数据结构(链表、栈、队列、哈希表、二叉树);了解部分数据结构的基本原理（B/B+树、红黑树等);参考DPDK文档亲手实践过无锁数据结构。
     - desc: 操作系统：熟悉Linux操作系统及其常见问题定位方法和工具包。熟悉Linux环境下的多进程多线程编程，掌握常用线程间同步与互斥手段，对多线程性能调优有一定经验。
     - desc: 网络编程：有一定的网络编程通讯协议设计经验，掌握FEC编码，了解Reed-Solomon编码原理。了解五大IO模型以及常见多路复用技术。
     - desc: 其他：了解常见设计模式，有MySQL及Redis使用经验,了解MySQL查询性能优化相关技术。熟练使用vim、gdb、git、cmake、meson等常见工具链
